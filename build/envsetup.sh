function __print_aosvp_functions_help() {
cat <<EOF
...
- aosvpgerrit:   A Git wrapper that fetches/pushes patch from/to LineageOS Gerrit Review.
- aosvprebase:   Rebase a Gerrit change and push it again.
- aosvpremote:   Add git remote for LineageOS Gerrit Review.
...
function breakfast()
{
    target=$1
    local variant=$2
    AOSVP_DEVICES_ONLY="true"
    unset LUNCH_MENU_CHOICES
    add_lunch_combo full-eng
    for f in `/bin/ls vendor/aosvp/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
...
{
    if [ "$OUT" ] ; then
        MODVERSION=$(get_build_var AOSVP_VERSION)
        ...
        if (adb shell getprop ro.aosvp.device | grep -q "$AOSVP_BUILD"); then
...
EOF
            ...
            echo "The connected device does not appear to be $AOSVP_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function aosvpremote()
{
    ...
    git remote rm aosvp 2> /dev/null
    ...
    local AOSVP_USER=$(git config --get review.review.aosvpos.org.username)
    if [ -z "$AOSVP_USER" ]
    then
        git remote add aosvp ssh://review.aosvpos.org:29418/$PFX$GERRIT_REMOTE
    else
        git remote add aosvp ssh://$AOSVP_USER@review.aosvpos.org:29418/$PFX$GERRIT_REMOTE
    fi
    echo "Remote 'aosvp' created"
}
...
function installboot()
{
    ...
    if (adb shell getprop ro.aosvp.device | grep -q "$AOSVP_BUILD");
    then
...
function makerecipe() {
    ...
    then
        pwd
        aosvpremote
        git push aosvp HEAD:refs/heads/'$1'
    fi
    '
}

function aosvpgerrit() {
    ...
    local user=`git config --get review.review.aosvpos.org.username`
...
function aosvprebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        ...
        echo "      aosvprebase <path to project> <patch IDs on Gerrit>"
    ...
    git fetch "http://review.aosvpos.org/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
...
function dopush()
{
    ...
    if (adb shell getprop ro.aosvp.device | grep -q "$AOSVP_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function repopick() {
    T=$(gettop)
    $T/vendor/aosvp/build/tools/repopick.py $@
}

function fixup_common_out_dir() {
    common_out_dir=$(get_build_var OUT_DIR)/target/common
    target_device=$(get_build_var TARGET_DEVICE)
    if [ ! -z $AOSVP_FIXUP_COMMON_OUT ]; then
