...
function setup_vendor() {
    ...
    export AOSVP_ROOT="$3"
    if [ ! -d "$AOSVP_ROOT" ]; then
        echo "\$AOSVP_ROOT must be set and valid before including this script!"
        exit 1
    fi

    export OUTDIR=vendor/"$VENDOR"/"$DEVICE"
    if [ ! -d "$AOSVP_ROOT/$OUTDIR" ]; then
        mkdir -p "$AOSVP_ROOT/$OUTDIR"
    fi
    ...
    export PRODUCTMK="$AOSVP_ROOT"/"$OUTDIR"/"$VNDNAME"-vendor.mk
    export ANDROIDMK="$AOSVP_ROOT"/"$OUTDIR"/Android.mk
    export BOARDMK="$AOSVP_ROOT"/"$OUTDIR"/BoardConfigVendor.mk
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$AOSVP_ROOT"/vendor/aosvp/build/tools/smali/baksmali.jar
        export SMALIJAR="$AOSVP_ROOT"/vendor/aosvp/build/tools/smali/smali.jar
    fi
    ...
    if [ ! -f "$AOSVP_TARGET" ]; then
        return;
    fi

    if grep "classes.dex" "$AOSVP_TARGET" >/dev/null; then
...
function extract() {
    ...
    local SRC="$2"
    local OUTPUT_ROOT="$AOSVP_ROOT"/"$OUTDIR"/proprietary
    ...
    if [ -f "$SRC" ] && [ "${SRC##*.}" == "zip" ]; then
        DUMPDIR="$AOSVP_ROOT"/system_dump
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            # If OTA is block based, extract it.
                ...
                python "$AOSVP_ROOT"/vendor/aosvp/build/tools/sdat2img.py "$DUMPDIR"/system.transfer.list "$DUMPDIR"/system.new.dat "$DUMPDIR"/system.img 2>&1
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$AOSVP_ROOT"/"$OUTDIR"/radio
